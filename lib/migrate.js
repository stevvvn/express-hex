...
const verbs = {
	...
	'create': (path, [ type, descr ], cb) => {
		...
		const
			...
			folder = `${base}/${type}`,
			file = `${ folder }/${(new Date).toISOString().replace(/[.:]/g, '-')}-${descr.toLowerCase().replace(/[^a-z0-9]+/g, '-')}.js`,
...
function getRecord(path) {
	if (!fs.existsSync(`${path}/migrations`)) {
		fs.mkdirSync(`${path}/migrations`);
	}
	if (!fs.existsSync(`${path}/migrations/record.json`)) {
		fs.writeFileSync(`${path}/migrations/record.json`, '{}');
		return {};
	}
	return JSON.parse(fs.readFileSync(`${path}/migrations/record.json`, { 'encoding': 'utf8' }));
}
