...
/// @TODO up and down need to not call the next migration before the prior has resolved, to keep the record straight
/// @TODO record dates probably also need an index for multi-migration run ordering, so down with no arguments works
const verbs = {
	'up': (path, [ target ]) => {
		const work = [], record = getRecord(path), dt = (new Date).toISOString(), ctx = hex.context();
		...
		targets.forEach((target) => {
			console.log('\t/\\ ', pth.resolve(target));
			const rv = require(target).up(ctx);
			// log resolution after promises resolve
			// setRecord() called individually so partially-complete runs reflect proper record
			if (rv && rv instanceof Promise) {
				work.push(new Promise((resolve, reject) => {
					rv.then(() => {
						...
						console.error('in', pth.resolve(target), '-');
						reject(err);
					});
				}));
			}
			else {
				// log resolution for synchronous migrations
				record[pth.basename(target)] = (new Date).toISOString();
				setRecord(path, record);
			}
		});
