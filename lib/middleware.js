...
module.exports = ({ log, launchPath, conf, app, express, action }: { log: Logger, launchPath: string, conf: Conf, app: any, express: any, action?: (mwPath: string, params: any) => void }): Promise<void> => {
	let earlyErr, launchApp;
	const required = {},
		bundles = {},
		appPaths = { 'hex': `${__dirname}/..` },
		_action = action ? action : (path, params) => { return require(path)(params); }
		;
	...
	return new Promise(async (resolve, reject) => {
		...
		const load = async () => {
			if (Object.keys(required).length === 0) {
				_action(`${__dirname}/../middleware/finalize.js`, { app });
				return resolve();
			}
			const round = [], work = [];
			Object.keys(required).forEach((mwName) => {
				...
				if (!mw.deps && !mw.after) {
					round.push(new Promise(async (mwres, mwrej) => {
						...
						try {
							const impl = _action(`${appPaths[mw.app]}/middleware/${mw.name}`, { log, conf, app, express });
