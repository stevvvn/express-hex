...
module.exports = ({ log, launchPath, conf, app, http, express}: Context, action?: (mwPath: string, params: any) => void): Promise<void> => {
	...
	const required = {},
		...
		staticBase = conf.get('staticBase', '/')
		;

	app.locals.initDeferred = [];
	...
	try {
		...
		const requireMiddleware = (path: string, appName: string = '') => {
			...
			try {
				...
				throw new Error(`error parsing middleware in ${path}: ${ex}`);
			}
			if (fs.existsSync(`${ dirname(path) }/node_modules`)) {
				module.paths.push(`${ dirname(path) }/node_modules`);
			}
			if (!mwDefs[appName]) {
				mwDefs[appName] = {};
			}
      ...
      conf.push('paths.base', appPaths[appName]);
		...
		launchApp = requireMiddleware(
      fs.existsSync(`${launchPath}/dist`)
        ? `${launchPath}/dist/middleware.js`
        : `${launchPath}/middleware.js`
    );
		...
		const getDef = (qmw:string): AnnotatedMiddlewareDef => {
			...
			if (!mwDefs[app]) {
				requireMiddleware(`${app}/dist/middleware.js`, app);
			}
      if (!mwDefs[app]) {
		...
		const requireMw = (mw: string|AnnotatedMiddlewareDef) => {
			...
			// special name "_default" is for apps that don't actually implement middleware (when deps do all the work)
			// it's used to depend on things without having to also make an empty middleware just to pin the deps to
			if (mw.name !== '_default') {
